...
## v1.0.7
- 保存セットの右クリックメニューから、XIVGear / BisBuddy へのエクスポートと削除を行えるようにしました。
- 圧縮表示された装備一覧から、エクスポート時に使用する具体装備を選択できるようにしました。
- 保存セット切り替え、装備一覧の再描画、ワーカーエラー時の復帰処理を見直し、表示や操作の安定性を改善しました。
- 終了時の保存処理を整理し、設定ファイルの保存を atomic write 化して破損しにくくしました。
- レベルシンク条件変更時の一覧更新や、保存セット削除後の表示不整合などの不具合を修正しました。
